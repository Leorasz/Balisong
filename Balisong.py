...
class Balisong:
    ...
    class Node:  # node class for the causlang interpreter
        def __init__(self, name):
            ...
            if (
                name[0] == "+"
            ):  # blue=on, red=off because inhibited, purple=off because nothing causing it
                self.color = "blue"
            elif name[0] == "-":
                self.color = "red"
            else:
    ...
    def interpretCauslang(self, inp):
        ...
        for relationship in relationships:
            ...
            if (
                components[0] not in nodenames and components[0][1:] not in nodenames
            ):  # makes new node
                if self.DEBUG >= 2:
            ...
            if components[0][0] == "+" or components[0][0] == "-":
                components[0] = components[0][
                    1:
                ]  # get rid of color setters to keep names consistent
    ...
    class Node2:  # node class for the causlang interpreter
        def __init__(self, name):
            self.name = name
            self.children = []
            self.color = "purple"
    ...
    def interpretCauslang2(self, inp):
        """
        New to Causlang2:
        -Inferred prefix (no +causer:x:x or -causer:x:x)
        -Causes by default, no inhibitions (causer:affected not causer:affected:+)
        """
        nodenames = set()
        nodes = []
        children = set()
        ...
        for relationship in relationships:
            ...
            if ":" in relationship:
                ...
                if relationship[1:] in nodenames:
                    ...
                    turnedOff = self.Node2(relationship[1:])
            ...
            if components[0] not in nodenames:  # makes new node
                if self.DEBUG >= 2:
                    print(f"Causer {components[0]} not in nodenames, creating new node")
                causer = self.Node2(components[0])
                nodenames.add(components[0])
                nodes.append(causer)
            ...
            if components[1] not in nodenames:
                ...
                nodenames.add(components[1])
                affected = self.Node2(components[1])
                nodes.append(affected)
                children.add(components[1])
            else:
                affected = getNode(components[1])
    ...
    def performCausalInference(self, text, scenario):  # do the whole thing
        ...
        with open(
            "long_strings.json", "r"
        ) as f:  # get long strings, easier to store them in a different file
            strings = json.load(f)
        if self.DEBUG >= 2:
        ...
        if not self.final_validation or self.validateResult(
            strings["basicSystem"], comparisonValidatorInput, grading=False
        ):
    ...
    def performCausalInference2(self, text, scenario):  # do the whole thing
        ...
        initialResults = self.interpretCauslang2(initialGraph)  # computing the effects
        scenarioResults = self.interpretCauslang2(scenarioGraph)
        if self.DEBUG >= 1:
            print("Results from both graphs calculated, now onto comparing them.")
...
print(bl.performCausalInference2(text, scenario))
